var uipVue = (function (exports) {
  ...
  const isNonTrackableKeys = /*#__PURE__*/ makeMap(`__proto__,__v_isRef,__isuipVue`);
  ...
  function warn(msg, ...args) {
    ...
    if (appWarnHandler) {
      ...
      const warnArgs = [`[uipVue warn]: ${msg}`, ...args];
      /* istanbul ignore if */
      if (
        trace.length &&
  ...
  const ErrorTypeStrings = {
    ...
    [14 /* SCHEDULER */]: "scheduler flush. This is likely a uipVue internals bug. " + "Please open an issue at https://new-issue.uipVuejs.org/?repo=uipVuejs/uipVue-next",
  };
  ...
  // it easier to be used in toolings like uipVue-loader
  ...
  // to be set so that its instances can be registered / removed.
  {
    ...
    globalObject.__uipVue_HMR_RUNTIME__ = {
  ...
  function createRecord(id, component) {
    if (!component) {
      warn(`HMR API usage is out of date.\n` + `Please upgrade uipVue-loader/vite/rollup-plugin-uipVue or other relevant ` + `dependency that handles uipVue SFC compilation.`);
      component = {};
    }
    if (map.has(id)) {
      return false;
  ...
  function tryWrap(fn) {
    return (id, arg) => {
      try {
        ...
        console.error(e);
        console.warn(`[HMR] Something went wrong during uipVue component hot-reload. ` + `Full reload required.`);
      }
    };
  }
      ...
      const app = (context.app = {
        ...
        mount(rootContainer, isHydrate, isSVG) {
          if (!isMounted) {
            ...
            rootContainer.__uipVue_app__ = app;
            {
        ...
        unmount() {
          if (isMounted) {
            ...
            delete app._container.__uipVue_app__;
          } else {
  ...
  function startMeasure(instance, type) {
    if (instance.appContext.config.performance && isSupported()) {
      perf.mark(`uipVue-${type}-${instance.uid}`);
    }
  }
  function endMeasure(instance, type) {
    if (instance.appContext.config.performance && isSupported()) {
      const startTag = `uipVue-${type}-${instance.uid}`;
  ...
  function baseCreateRenderer(options, createHydrationFns) {
    {
      ...
      target.__uipVue__ = true;
      setDevtoolsHook(target.__uipVue_DEVTOOLS_GLOBAL_HOOK__);
    }
    const {
      insert: hostInsert,
      remove: hostRemove,
    ...
    const mountElement = (vnode, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized) => {
      ...
      }
      {
        ...
        Object.defineProperty(el, "__uipVueParentComponent", {
        ...
        if (!target) {
          warn(
            `Failed to locate Teleport target with selector "${targetSelector}". ` +
              ...
              `ideally should be outside of the entire uipVue component tree.`
          );
        }
        return target;
      }
  ...
  function _createVNode(type, props = null, children = null, patchFlag = 0, dynamicProps = null, isBlockNode = false) {
    ...
    if (shapeFlag & 4 /* STATEFUL_COMPONENT */ && isProxy(type)) {
      type = toRaw(type);
      warn(
        `uipVue received a Component which was made a reactive object. This can ` +
  ...
  function applyOptions(instance, options, deferredData = [], deferredWatch = [], deferredProvide = [], asMixin = false) {
    ...
    // options initialization order (to be consistent with uipVue 2):
  ...
  /**
   * #2437 In uipVue 3, functional components do not have a public instance proxy but
  ...
  const PublicInstanceProxyHandlers = {
    get({ _: instance }, key) {
      ...
      // let @uipVue/reactivity know it should never observe uipVue public instances.
      if (key === "__v_skip" /* SKIP */) {
        return true;
      }
      // for internal formatters to know that this is a uipVue instance
      if (key === "__isuipVue") {
        return true;
      }
      // data / props / ctx
      ...
      if (publicGetter) {
        ...
        // css module (injected by uipVue-loader)
  ...
  const RuntimeCompiledPublicInstanceProxyHandlers = extend({}, PublicInstanceProxyHandlers, {
    ...
    has(_, key) {
      ...
      if (!has && PublicInstanceProxyHandlers.has(_, key)) {
        warn(`Property ${JSON.stringify(key)} should not start with _ which is a reserved prefix for uipVue internals.`);
      }
      return has;
    },
  });
  ...
  function exposeSetupStateOnRenderContext(instance) {
    ...
    Object.keys(toRaw(setupState)).forEach((key) => {
      if (key[0] === "$" || key[0] === "_") {
        warn(`setup() return property ${JSON.stringify(key)} should not start with "$" or "_" ` + `which are reserved prefixes for uipVue internals.`);
        return;
      }
      Object.defineProperty(ctx, key, {
        enumerable: true,
  ...
  function finishComponentSetup(instance, isSSR) {
    ...
    if (!Component.render && instance.render === NOOP && !isSSR) {
      ...
      if (!compile && Component.template) {
        warn(`Component provided template option but ` + `runtime compilation is not supported in this build of uipVue.` + ` Use "uipVue.global.js" instead.` /* should not happen */);
      } else {
  ...
  function initCustomFormatter() {
    ...
    const uipVueStyle = { style: "color:#3ba776" };
    ...
    const formatter = {
      header(obj) {
        ...
        if (obj.__isuipVue) {
          return ["div", uipVueStyle, `uipVueInstance`];
        } else if (isRef(obj)) {
          return ["div", {}, ["span", uipVueStyle, genRefFlag(obj)], "<", formatValue(obj.value), `>`];
        } else if (isReactive(obj)) {
          return ["div", {}, ["span", uipVueStyle, "Reactive"], "<", formatValue(obj), `>${isReadonly(obj) ? ` (readonly)` : ``}`];
        } else if (isReadonly(obj)) {
          return ["div", {}, ["span", uipVueStyle, "Readonly"], "<", formatValue(obj), ">"];
        }
        return null;
      },
      hasBody(obj) {
        return obj && obj.__isuipVue;
      },
      body(obj) {
        if (obj && obj.__isuipVue) {
  ...
  /**
   * SSR utils for \@uipVue/server-renderer. Only exposed in cjs builds.
   * @internal
   */
  const ssrUtils = null;
  ...
  // in case the user only imports reactivity utilities from uipVue.
  let renderer;
        ...
        set() {
          warn(
            `The \`isCustomElement\` config option is only respected when using the runtime compiler.` +
              `If you are using the runtime-only build, \`isCustomElement\` must be passed to \`@uipVue/compiler-dom\` in the build setup instead` +
              `- for example, via the \`compilerOptions\` option in uipVue-loader: https://uipVue-loader.uipVuejs.org/options.html#compileroptions.`
          );
        },
      });
    }
  ...
  // Name mapping for runtime helpers that need to be imported from 'uipVue' in
    ...
    if (!context.inVPre && /^(v-|:|@|#)/.test(name)) {
      ...
      if (match[2]) {
        ...
        if (content.startsWith("[")) {
          ...
          // #1241 special case for v-slot: uipVuetify relies extensively on slot
          // names containing dots. v-slot doesn't have any modifiers and uipVue 2.x
  ...
  function walk(node, context, doNotHoistNode = false) {
    ...
    // Some transforms, e.g. transformAssetUrls from @uipVue/compiler-sfc, replaces
    // static bindings with expressions. These expressions are guaranteed to be
    // constant so they are still eligible for hoisting, but they are only
    // available at runtime and therefore cannot be evaluated ahead of time.
    // This is only a concern for pre-stringification (via transformHoist by
    // @uipVue/compiler-dom), but doing it here allows us to perform only one full
  ...
  function createCodegenContext(
    ast,
    {
      ...
      sourceMap = false,
      filename = `template.uipVue.html`,
      scopeId = null,
      optimizeImports = false,
      runtimeGlobalName = `uipVue`,
      runtimeModuleName = `uipVue`,
      ssr = false,
    }
  ) {
    const context = {
  ...
  function generate(ast, options = {}) {
    ...
    if (useWithBlock) {
      ...
      if (hasHelpers) {
        push(`const { ${ast.helpers.map((s) => `${helperNameMap[s]}: _${helperNameMap[s]}`).join(", ")} } = _uipVue`);
        push(`\n`);
        newline();
      }
    }
  ...
  function genFunctionPreamble(ast, context) {
    ...
    const uipVueBinding = runtimeGlobalName;
    ...
    if (ast.helpers.length > 0) {
      {
        // "with" mode.
        // save uipVue in a separate variable to avoid collision
        push(`const _uipVue = ${uipVueBinding}\n`);
        ...
        if (ast.hoists.length) {
          const staticHelpers = [CREATE_VNODE, CREATE_COMMENT, CREATE_TEXT, CREATE_STATIC]
            ...
            .map(aliasHelper)
            .join(", ");
          push(`const { ${staticHelpers} } = _uipVue\n`);
        }
      }
    }
  ...
  // @uipVue/compiler-dom can export `compile` while re-exporting everything else.
  ...
  function compileToFunction(template, options) {
    ...
    // In the global build we know `uipVue` is available globally so we can avoid
    // the wildcard object.
