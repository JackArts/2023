...
include 'wilmer-instagram-helper.php';

/**
 * Class WilmerInstagramApi
 */
class WilmerInstagramApi {
	...
	const INSTAGRAM_CODE = 'wilmer_instagram_code';
	const INSTAGRAM_USER_ID = 'wilmer_instagram_user_id';
	const INSTAGRAM_TOKEN = 'wilmer_instagram_access_token';
	const INSTAGRAM_TOKEN_LONG_LIVED = 'wilmer_instagram_access_token_long_lived';
	const INSTAGRAM_TOKEN_LONG_LIVED_TIME = 'wilmer_instagram_access_token_long_lived_time';

	const FACEBOOK_CODE = 'wilmer_facebook_code';
	const FACEBOOK_TOKEN = 'wilmer_facebook_access_token';

	const CONNECTION_TYPE = 'wilmer_connection_type';
	...
	public function __construct() {
		...
		$this->helper = new WilmerInstagramHelper();
	}

	/**
	 * @return WilmerInstagramApi
	 */
	 ...
	 *
	 * @see WilmerInstagramApi::fetchData()
	 */
	 ...
	 * @return stdClass
	 *
	 * @see WilmerInstagramApi::instagramExchangeCodeForToken()
	 */
		...
		if ( $this->useTransients( $transient ) && get_transient( $transient['transient_name'] ) ) {
			...
			if ( get_option( self::CONNECTION_TYPE ) === 'instagram' ) {
				if ( get_option( self::INSTAGRAM_TOKEN_LONG_LIVED_TIME ) ) {
					$url = wp_remote_get( 'https://graph.instagram.com/me/media?fields=media_url,caption,permalink,media_type,thumbnail_url&access_token=' . $this->instagramAccessTokenLongLived . '&count=' . $limit );
				} else {
					$url = wp_remote_get('https://graph.instagram.com/me/media?fields=media_url,caption,permalink,media_type,thumbnail_url&access_token=' . $this->instagramAccessToken . '&count=' . $limit);
				}
