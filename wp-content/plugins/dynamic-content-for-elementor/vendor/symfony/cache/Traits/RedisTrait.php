...
use DynamicOOOS\Predis\Connection\Aggregate\RedisCluster;
...
trait RedisTrait
{
    ...
    private function init($redis, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller)
    {
        ...
        if (!$redis instanceof \Redis && !$redis instanceof \RedisArray && !$redis instanceof \RedisCluster && !$redis instanceof \DynamicOOOS\Predis\ClientInterface && !$redis instanceof RedisProxy && !$redis instanceof RedisClusterProxy) {
            throw new InvalidArgumentException(\sprintf('"%s()" expects parameter 1 to be Redis, RedisArray, RedisCluster or Predis\\ClientInterface, "%s" given.', __METHOD__, \is_object($redis) ? \get_class($redis) : \gettype($redis)));
        }
        if ($redis instanceof \DynamicOOOS\Predis\ClientInterface && $redis->getOptions()->exceptions) {
    ...
    public static function createConnection($dsn, array $options = [])
    {
        ...
        if (!\extension_loaded('redis') && !\class_exists(\DynamicOOOS\Predis\Client::class)) {
        ...
        if (isset($params['redis_sentinel']) && !\class_exists(\DynamicOOOS\Predis\Client::class)) {
        ...
        } elseif (\is_a($class, \DynamicOOOS\Predis\ClientInterface::class, \true)) {
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \DynamicOOOS\Predis\ClientInterface && $this->redis->getConnection() instanceof ClusterInterface) {
    ...
    protected function doClear($namespace)
    {
        ...
        $hosts = $this->getHosts();
        $host = \reset($hosts);
        if ($host instanceof \DynamicOOOS\Predis\Client && $host->getConnection() instanceof ReplicationInterface) {
        ...
        foreach ($hosts as $host) {
            ...
            if (!$host instanceof \DynamicOOOS\Predis\ClientInterface) {
            ...
            if (!\version_compare($info['redis_version'], '2.8', '>=')) {
                ...
                $args = $this->redis instanceof \DynamicOOOS\Predis\ClientInterface ? [0, $pattern] : [[$pattern], 0];
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \DynamicOOOS\Predis\ClientInterface ? $host->scan($cursor, 'MATCH', $pattern, 'COUNT', 1000) : $host->scan($cursor, $pattern, 1000);
    ...
    private function pipeline(\Closure $generator, $redis = null) : \Generator
    {
        ...
        if ($redis instanceof RedisClusterProxy || $redis instanceof \RedisCluster || $redis instanceof \DynamicOOOS\Predis\ClientInterface && $redis->getConnection() instanceof RedisCluster) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \DynamicOOOS\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \DynamicOOOS\Predis\ClientInterface) {
        ...
        if (!$redis instanceof \DynamicOOOS\Predis\ClientInterface && 'eval' === $command && $redis->getLastError()) {
        ...
        if ($this->redis instanceof \DynamicOOOS\Predis\ClientInterface) {
            ...
            if ($connection instanceof ClusterInterface && $connection instanceof \Traversable) {
                ...
                foreach ($connection as $c) {
                    $hosts[] = new \DynamicOOOS\Predis\Client($c);
                }
            }
