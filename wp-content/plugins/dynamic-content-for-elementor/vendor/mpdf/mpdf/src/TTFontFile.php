...
use DynamicOOOS\Mpdf\Fonts\GlyphOperator;
...
if (!\defined('_OTL_OLD_SPEC_COMPAT_2')) {
    \define('_OTL_OLD_SPEC_COMPAT_2', \true);
}
...
// mPDF 5.7.1
if (!\function_exists('DynamicOOOS\\Mpdf\\unicode_hex')) {
...
class TTFontFile
{
    ...
    public function getMetrics($file, $fontkey, $TTCfontID = 0, $debug = \false, $BMPonly = \false, $useOTL = 0)
    {
        ...
        if (!$this->fh) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Unable to open font file "%s"', $file));
        }
        $this->_pos = 0;
        $this->charWidths = '';
        $this->glyphPos = [];
        ...
        if ($version === 0x4f54544f) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Fonts with postscript outlines are not supported (%s)', $file));
        }
        if ($version === 0x74746366 && !$TTCfontID) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('TTCfontID for a TrueType Collection is not defined in mPDF "fontdata" configuration (%s)', $file));
        }
        if (!\in_array($version, [0x10000, 0x74727565], \true) && !$TTCfontID) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Not a TrueType font: version=%s)', $version));
        }
        if ($TTCfontID > 0) {
            $this->version = $version = $this->read_ulong();
            // TTC Header version now
            if (!\in_array($version, [0x10000, 0x20000], \true)) {
                throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Error parsing TrueType Collection: version=%s - (%s)', $version, $file));
            }
        ...
        foreach ($this->tables as $t) {
            if ($t['length'] > 0 && $t['length'] < $this->maxStrLenRead) {
                ...
                if ($xchecksum != $checksum) {
                    throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('TTF file "%s": invalid checksum %s table: %s (expected %s)', $this->filename, \dechex($checksum[0]) . \dechex($checksum[1]), $t['tag'], \dechex($xchecksum[0]) . \dechex($xchecksum[1])));
                }
            }
        }
    }
    ...
    function getCTG($file, $TTCfontID = 0, $debug = \false, $useOTL = \false)
    {
        ...
        $this->fh = \fopen($file, 'rb');
        if (!$this->fh) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Unable to open file "%s"', $file));
        }
        $this->_pos = 0;
        $this->charWidths = '';
        $this->glyphPos = [];
        ...
        if ($useOTL) {
            ...
            for ($gid = 1; $gid < $numGlyphs; $gid++) {
                if (!isset($glyphToChar[$gid])) {
                    ...
                    if ($bctr > 0xf8ff) {
                        throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Font "%s" cannot map all included glyphs into Private Use Area U+E000-U+F8FF; cannot use useOTL on this font', $file));
                    }
    ...
    function getTTCFonts($file)
    {
        ...
        if ($version === 0x74746366) {
            ...
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf("Not a TrueType Collection: version=%s (%s)", $version, $file));
        }
        $this->numTTCFonts = $this->read_ulong();
        for ($i = 1; $i <= $this->numTTCFonts; $i++) {
            $this->TTCFonts[$i]['offset'] = $this->read_ulong();
    ...
    function extractInfo($debug = \false, $BMPonly = \false, $useOTL = 0)
    {
        ...
        if ($format != 0 && $format != 1) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException("Error loading font: Unknown name table format {$format} for font {$this->filename}");
        }
        ...
        for ($i = 0; $i < $numRecords; $i++) {
            ...
            if ($platformId == 3 && $encodingId == 1 && $languageId == 0x409) {
                ...
                if ($length % 2 != 0) {
                    throw new \DynamicOOOS\Mpdf\Exception\FontException("Error loading font: PostScript name is UTF-16BE string of odd length for font {$this->filename}");
                }
                $length /= 2;
                $N = '';
                while ($length > 0) {
                ...
                $this->_pos = $opos;
                $this->seek($opos);
            } elseif ($platformId == 1 && $encodingId == 0 && $languageId == 0) {
        ...
        if (!$psName) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException("Error loading font: Could not find PostScript font name '{$this->filename}'");
        }
        ...
        if ($debug) {
            ...
            if ($ver_maj != 1) {
                throw new \DynamicOOOS\Mpdf\Exception\FontException('Error loading font: Unknown head table version ' . $ver_maj . '.' . $ver_min);
            }
            $this->fontRevision = $this->read_ushort() . $this->read_ushort();
            $this->skip(4);
            $magic = $this->read_ulong();
            if ($magic !== 0x5f0f3cf5) {
                throw new \DynamicOOOS\Mpdf\Exception\FontException('Error loading font: Invalid head table magic ' . $magic);
            }
            $this->skip(2);
        } else {
            $this->skip(18);
        ...
        if ($glyphDataFormat != 0) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Error loading font: Unknown glyph data format %s', $glyphDataFormat));
        }
        // hhea metrics table
        ...
        if (isset($this->tables["OS/2"])) {
            ...
            $use_typo_metrics = ($fsSelection & 0x80) === 0x80;
        ...
        if ($debug) {
            ...
            if ($ver_maj < 1 || $ver_maj > 4) {
                throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Error loading font: Unknown post table version %s', $ver_maj));
            }
        } else {
            $this->skip(4);
        }
        ...
        if ($debug) {
            $ver_maj = $this->read_ushort();
            if ($ver_maj != 1) {
                throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Error loading font: Unknown hhea table version %s', $ver_maj));
            }
            $this->skip(28);
        } else {
            $this->skip(32);
        ...
        if ($metricDataFormat != 0) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Error loading font: Unknown horizontal metric data format "%s"', $metricDataFormat));
        }
        $numberOfHMetrics = $this->read_ushort();
        if ($numberOfHMetrics == 0) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException('Error loading font: Number of horizontal metrics is 0');
        }
        ...
        if ($debug) {
            $ver_maj = $this->read_ushort();
            if ($ver_maj != 1) {
                throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Error loading font: Unknown maxp table version %s', $ver_maj));
            }
        } else {
            $this->skip(4);
        }
        ...
        if (!$unicode_cmap_offset) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Font "%s" does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)', $this->filename));
        }
        $sipset = \false;
        $smpset = \false;
        $this->rtlPUAstr = '';
                ...
                if (!isset($glyphToChar[$gid])) {
                    ...
                    if ($bctr > 0xf8ff && $bctr < 0x2ceb0) {
                        if (!$BMPonly) {
                            ...
                            $this->sipset = $sipset = \true;
                            // forces subsetting; also ensure charwidths are saved
                            while (isset($charToGlyph[$bctr])) {
                                $bctr++;
                            }
                        } else {
                            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('The font "%s" does not have enough space to map all (unmapped) included glyphs into Private Use Area U+E000-U+F8FF', $names[1]));
                        }
                    }
                    $glyphToChar[$gid][] = $bctr;
                    $charToGlyph[$bctr] = $gid;
    ...
    function _getGDEFtables()
    {
        ...
        if (isset($this->tables["GDEF"])) {
            ...
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Unable to set font "%s" to use OTL as it does not include OTL tables (or at least not a GDEF table).', $this->filename));
        }
        $GSUB_offset = 0;
        $GPOS_offset = 0;
        $GSUB_length = 0;
    ...
    function _getClassDefinitionTable()
    {
        ...
        foreach ($GlyphByClass as $class => $glyphs) {
            \sort($GlyphByClass[$class], \SORT_STRING);
                ...
                */
                ...
                if ($Lookup[$i]['Type'] == 1) {
                    ...
                    if ($SubstFormat == 1) {
                        ...
                        throw new \DynamicOOOS\Mpdf\Exception\FontException("GPOS Lookup Type " . $Lookup[$i]['Type'] . ", Format " . $SubstFormat . " not supported (ttfontsuni.php).");
                    }
                    ...
                    throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Lookup Type "%s" not supported.', $Lookup[$i]['Type']));
                }
            }
        }
            ...
            for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++) {
                ...
                if ($Lookup[$i]['Type'] == 1) {
                    ...
                    if ($SubstFormat == 1) {
                        ...
                        throw new \DynamicOOOS\Mpdf\Exception\FontException("Lookup Type 5, SubstFormat 3 not tested. Please report this with the name of font used - " . $this->fontkey);
                    }
                } elseif ($Lookup[$i]['Type'] == 6) {
                    // Format 1: Simple Chaining Context Glyph Substitution  p255
                    if ($SubstFormat == 1) {
        ...
        foreach ($gsub as $st => $scripts) {
            foreach ($scripts as $t => $langsys) {
                ...
                if (\count($rtl) || \count($rphf) || \count($half) || \count($pref) || \count($blwf) || \count($pstf) || $finals) {
                    $font = ['rtlSUB' => $rtlSUB, 'finals' => $finals, 'rphf' => $rphf, 'half' => $half, 'pref' => $pref, 'blwf' => $blwf, 'pstf' => $pstf];
    ...
    function _checkGSUBignore($flag, $glyph, $MarkFilteringSet)
    {
        ...
        if (($flag & 0x8) == 0x8 && ($flag & 0xff00) == 0 && \strpos($this->GlyphClassMarks, $glyph)) {
            $ignore = \true;
        }
        if (($flag & 0x4) == 0x4 && \strpos($this->GlyphClassLigatures, $glyph)) {
            $ignore = \true;
        }
        if (($flag & 0x2) == 0x2 && \strpos($this->GlyphClassBases, $glyph)) {
            $ignore = \true;
        }
    ...
    function _getGSUBignoreString($flag, $MarkFilteringSet)
    {
        ...
        // Flag & 0x0010 = UseMarkFilteringSet
        if ($flag & 0x10) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException("This font " . $this->fontkey . " contains MarkGlyphSets - Not tested yet");
    ...
    */
    ...
    function _getCoverage($convert2hex = \true, $mode = 1)
    {
        $g = [];
        $ctr = 0;
    ...
    function makeSubset($file, &$subset, $TTCfontID = 0, $debug = \false, $useOTL = \false)
    {
        ...
        if (!$unicode_cmap_offset) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Font "%s" does not have Unicode cmap (platform 3, encoding 1, format 4, or platform 0 [any encoding] format 4)', $this->filename));
        }
        $glyphToChar = [];
        $charToGlyph = [];
        $this->getCMAP4($unicode_cmap_offset, $glyphToChar, $charToGlyph);
        ...
        if ($useOTL) {
            ...
            for ($gid = 1; $gid < $numGlyphs; $gid++) {
                if (!isset($glyphToChar[$gid])) {
                    ...
                    // Avoid overwriting a glyph already mapped in PUA
                    if ($bctr > 0xf8ff) {
                        throw new \DynamicOOOS\Mpdf\Exception\FontException($file . " : WARNING - Font cannot map all included glyphs into Private Use Area U+E000 - U+F8FF; cannot use useOTL on this font");
                    }
                    $glyphToChar[$gid][] = $bctr;
                    $charToGlyph[$bctr] = $gid;
                    $bctr++;
        ...
        foreach ($subset as $code) {
            if (isset($this->charToGlyph[$code])) {
                $subsetglyphs[$this->charToGlyph[$code]] = $code;
                // Old Glyph ID => Unicode
                $subsetCharToGlyph[$code] = $this->charToGlyph[$code];
        ...
        foreach ($subsetglyphs as $originalGlyphIdx => $uni) {
            ...
            } elseif (_RECALC_PROFILE && $glyphLen > 2 && $up[1] < 1 << 15 && $up[1] > 0) {
        ...
        if (_RECALC_PROFILE) {
            $head = $this->_set_short($head, 36, $xMinT);
            // for all glyph bounding boxes
            $head = $this->_set_short($head, 38, $yMinT);
            // for all glyph bounding boxes
            $head = $this->_set_short($head, 40, $xMaxT);
            // for all glyph bounding boxes
            $head = $this->_set_short($head, 42, $yMaxT);
            // for all glyph bounding boxes
        ...
        if (_RECALC_PROFILE) {
            $maxp = $this->_set_ushort($maxp, 6, $maxPoints);
            // points in non-compound glyph
            $maxp = $this->_set_ushort($maxp, 8, $maxContours);
            // contours in non-compound glyph
            $maxp = $this->_set_ushort($maxp, 10, $maxComponentPoints);
            // points in compound glyph
            $maxp = $this->_set_ushort($maxp, 12, $maxComponentContours);
            // contours in compound glyph
            $maxp = $this->_set_ushort($maxp, 28, $maxComponentElements);
            // number of glyphs referenced at top level
            $maxp = $this->_set_ushort($maxp, 30, $maxComponentDepth);
            // levels of recursion, set to 0 if font has only simple glyphs
        }
        $this->add('maxp', $maxp);
        // OS/2 - OS/2
        if (isset($this->tables['OS/2'])) {
            ...
            if (_RECALC_PROFILE) {
                ...
                $os2 = $this->_set_ushort($os2, 46, $nonBMP);
    ...
    function makeSubsetSIP($file, &$subset, $TTCfontID = 0, $debug = \false, $useOTL = 0)
    {
        ...
        if ($TTCfontID > 0) {
            ...
            if (!\in_array($version, [0x10000, 0x20000])) {
                throw new \DynamicOOOS\Mpdf\Exception\FontException("ERROR - Error parsing TrueType Collection: version=" . $version . " - " . $file);
            }
            $this->numTTCFonts = $this->read_ulong();
            for ($i = 1; $i <= $this->numTTCFonts; $i++) {
                $this->TTCFonts[$i]['offset'] = $this->read_ulong();
        ...
        if (!$unicode_cmap_offset) {
            throw new \DynamicOOOS\Mpdf\Exception\FontException(\sprintf('Font "%s" does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)', $file));
        }
        // Format 12 CMAP does characters above Unicode BMP i.e. some HKCS characters U+20000 and above
        if ($format == 12) {
            $this->maxUniChar = 0;
    ...
    function getGlyphData($originalGlyphIdx, &$maxdepth, &$depth, &$points, &$contours)
    {
        ...
        } elseif ($this->glyphdata[$originalGlyphIdx]['nContours'] > 0 && $depth > 0) {
            // simple
    ...
    function getLOCA($indexToLocFormat, $numGlyphs)
    {
        ...
        if ($indexToLocFormat == 0) {
            ...
            throw new \DynamicOOOS\Mpdf\Exception\FontException('Unknown location table format ' . $indexToLocFormat);
        }
    }
    /**
     * CMAP Format 4
    ...
    function endTTFile(&$stm)
    {
        ...
        // Header
        if (_TTF_MAC_HEADER) {
            $stm .= \pack('Nnnnn', 0x74727565, $numTables, $searchRange, $entrySelector, $rangeShift);
            // Mac
        } else {
            $stm .= \pack('Nnnnn', 0x10000, $numTables, $searchRange, $entrySelector, $rangeShift);
            // Windows
        }
        // Table directory
        $tables = $this->otables;
        ...
        if ($useOTL) {
            ...
            for ($gid = 1; $gid < $numGlyphs; $gid++) {
                if (!isset($glyphToChar[$gid])) {
                    ...
                    if ($bctr > 0xf8ff) {
                        throw new \DynamicOOOS\Mpdf\Exception\FontException("Problem. Trying to repackage TF file; not enough space for unmapped glyphs");
                    }
                    $glyphToChar[$gid][] = $bctr;
                    $charToGlyph[$bctr] = $gid;
                    $bctr++;
