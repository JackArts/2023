...
class Save extends \ElementorPro\Modules\Forms\Classes\Action_Base
{
    ...
    // first DCE form action to be run, because if there is an error it stops the other actions by dying. Not sure if it's a great idea.
    ...
    public function register_settings_section($widget)
    {
        ...
        $widget->add_control('dce_form_save_type_user_email', ['label' => __('User Email', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::TEXT, 'default' => '[field id="email"]', 'condition' => ['dce_form_save_type' => 'user', 'dce_form_save_override' => ''], 'label_block' => 'true']);
        $widget->add_control('dce_form_save_type_user_pass', ['label' => __('User Password', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::TEXT, 'default' => '[field id="user_pass"]', 'condition' => ['dce_form_save_type' => 'user', 'dce_form_save_override' => ''], 'label_block' => 'true']);
    ...
    private function save($record, $settings = null, $ajax_handler = null)
    {
        ...
        switch ($settings['dce_form_save_type']) {
            ...
            case 'user':
                ...
                $settings['dce_form_save_type_user_pass'] = $settings['dce_form_save_type_user_pass'] ? Helper::get_dynamic_value($settings['dce_form_save_type_user_pass'], $fields) : wp_generate_password();
                ...
                $error_msg = $settings['dce_form_save_type_user_error'] ? $settings['dce_form_save_type_user_error'] : \ElementorPro\Modules\Forms\Classes\Ajax_Handler::get_default_message(\ElementorPro\Modules\Forms\Classes\Ajax_Handler::SUBSCRIBER_ALREADY_EXISTS, $settings);
                if (!$obj_id || !$settings['dce_form_save_override']) {
