class Sketch {
  ...
  hoverEvent(){
    var tlh = new TimelineMax({ paused: true, reversed: true });
    //tl.to(this, 0.5, { paddingLeft: "+=100px" });
    tlh.to(this.material.uniforms.progress,this.duration,{
      value:1,
    ...
    this.clicker.addEventListener('mouseenter',()=>{
      tlh.reversed() ? tlh.play() : tlh.reverse();
    });
  ...
  resizeControl(){
    ...
    if( this.oldcontainertWidth != this.container.offsetWidth || this.oldcontainertHeight != this.container.offsetHeight) this.resize();
...
*/  
    if(elementorFrontend.isEditMode()) this.resizeControl();
