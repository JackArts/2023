var dce_crs=null;var Widget_DCE_Dynamicposts_crossroadsslideshow_Handler=function($scope,$){var elementSettings=dceGetElementSettings($scope);var id_scope=$scope.attr('data-id');var splitter;var split;const MathUtils={lineEq:(y2,y1,x2,x1,currentVal)=>{var m=(y2-y1)/(x2-x1),b=y1-m*x1;return m*currentVal+b},lerp:(a,b,n)=>(1-n)*a+n*b,distance:(x1,x2,y1,y2)=>{var a=x1-x2;var b=y1-y2;return Math.hypot(a,b)},randomNumber:(min,max)=>Math.floor(Math.random()*(max-min+1)+min)};let winsize;const calcWinsize=()=>winsize={width:window.innerWidth,height:window.innerHeight};calcWinsize();window.addEventListener('resize',calcWinsize);const getMousePos=(ev)=>{let posx=0;let posy=0;if(!ev)ev=window.event;if(ev.pageX||ev.pageY){posx=ev.pageX;posy=ev.pageY}else if(ev.clientX||ev.clientY){posx=ev.clientX+body.scrollLeft+docEl.scrollLeft;posy=ev.clientY+body.scrollTop+docEl.scrollTop}
...
const revealer=new Revealer(document.querySelector('.dce-revealer__inner'));new Slideshow(document.querySelector('.dce-grid--slideshow'));imagesLoaded(document.querySelectorAll('.dce-img'),{background:!0},()=>document.body.classList.remove('loading'));[...document.querySelectorAll('.frame__mode input[type="radio"]')].forEach(radio=>radio.addEventListener('click',()=>{document.body.classList[radio.parentNode.classList.contains('frame__mode-item--dark')?'add':'remove']('dark-mode')}))};jQuery(window).on('elementor/frontend/init',function(){elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamicposts-v2.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-woo-products-cart.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamic-woo-products.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamic-show-favorites.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-my-posts.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-sticky-posts.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-search-results.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler)})
