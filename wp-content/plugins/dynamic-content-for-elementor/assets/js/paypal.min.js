"use strict";function initializePayPalButtons(w,$scope){let buttons=w.querySelector(".dce-paypal-buttons")
...
let buttonsID=`#${buttons.getAttribute('id')}`;let orderName=buttons.getAttribute('data-name');let nameFieldId;if(!orderName){nameFieldId=buttons.getAttribute('data-name-field-id')}
let orderCurrency=buttons.getAttribute('data-currency');let orderValue=buttons.getAttribute('data-value');let valueFieldId;if(!orderValue){valueFieldId=buttons.getAttribute('data-value-field-id')}
let description=buttons.getAttribute('data-description');let sku=buttons.getAttribute('data-sku');let height=parseInt(buttons.getAttribute('data-height'));let layout=buttons.getAttribute('data-layout');paypal.Buttons({style:{layout:layout,height:height,},createOrder:function(data,actions){if(!orderValue){let fd=new FormData($scope.find('form')[0]);orderValue=fd.get(`form_fields[${valueFieldId}]`)}
if(!orderName){let fd=new FormData($scope.find('form')[0]);orderName=fd.get(`form_fields[${nameFieldId}]`)}
