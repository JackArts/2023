(function($){var WidgetElements_SvgDistortionHandler=function($scope,$){var elementSettings=dceGetElementSettings($scope);var id_scope=$scope.attr('data-id');var imgDisplacment=elementSettings.displacement_image.url;var image_url=$scope.find('.dce_distortion').attr('data-dispimage');if(!image_url){return}
...
var svg_trigger=elementSettings.svg_trigger;var tl=new gsap.timeline({repeat:-1,repeatDelay:animation_delay});var tli=new gsap.timeline({repeat:-1,repeatDelay:animation_delay});var interrompi=function(){tl.pause(0);tli.pause(0);is_running=!1};var ferma=function(){tl.pause();tli.pause();is_running=!1};var riproduci=function(){tl.play();tli.play();is_running=!0};var inverti=function(){tl.reverse();tli.reverse();is_running=!0}
...
requestAnimationFrame(repeatOften)};var moveFnComplete=function(){random_val_1=random_val_2;random_val_2=getRandomValue(scaleMap_rand_min,scaleMap_rand_max);createAnimation(!0)};function createAnimation($random=!1){if($random){tl=new gsap.timeline({repeat:0});tl.to(feDisp,{duration:animation_speed,onComplete:moveFnComplete,attr:{scale:random_val_1},ease:easeFunction},0).to(feDisp,{duration:animation_speed,attr:{scale:random_val_2},ease:easeFunction},animation_speed)}else{tl.to(feDisp,{duration:animation_speed,attr:{scale:0},ease:easeFunction},0).to(feDisp,{duration:animation_speed,attr:{scale:scaleMap},ease:easeFunction},animation_speed)}
is_running=!0;if(run=='paused'&&elementorFrontend.isEditMode()){ferma()}else{riproduci()}}
var mouseenterFn=function(){tl=new gsap.timeline({repeat:0});tli=new gsap.timeline({repeat:0});tl.to(feDisp,{duration:animation_speed,attr:{scale:scaleMapTo},ease:easeFunction},0);tli.to(feImage,{duration:animation_speed,attr:{x:posImageTo,y:posImageTo,width:scaleImageTo,height:scaleImageTo},ease:easeFunction},0)};var mouseleaveFn=function(){tl=new gsap.timeline({repeat:0});tli=new gsap.timeline({repeat:0});tl.to(feDisp,{duration:animation_speed,attr:{scale:scaleMap},ease:easeFunction},0);tli.to(feImage,{duration:animation_speed,attr:{x:posImage,y:posImage,width:scaleImage,height:scaleImage},ease:easeFunction},0)};var active_scrollAnalysi=function($el){if($el){tl=new gsap.timeline({repeat:0,paused:!0,});var runAnim=function(dir){if(dir=='down'){tl.to(feDisp,{duration:animation_speed,attr:{scale:scaleMapTo},ease:easeFunction},animation_delay);tli.to(feImage,{duration:animation_speed,attr:{x:posImageTo,y:posImageTo,width:scaleImageTo,height:scaleImageTo},ease:easeFunction},animation_delay);tl.restart();tli.restart()}else if(dir=='up'){tl.to(feDisp,{duration:animation_speed,attr:{scale:scaleMap},ease:easeFunction},animation_delay);tli.to(feImage,{duration:animation_speed,attr:{x:posImage,y:posImage,width:scaleImage,height:scaleImage},ease:easeFunction},animation_delay);tl.restart();tli.restart()}};var waypointOptions={triggerOnce:!1,continuous:!0};elementorFrontend.waypoint($($el),runAnim,waypointOptions)}};if(elementSettings.svg_trigger=='animation'){createAnimation(random_animation);if(elementorFrontend.isEditMode())playShapeEl()}else if(elementSettings.svg_trigger=='rollover'){$('.elementor-element[data-id='+id_scope+'] svg, .'+elementSettings.id_svg_class+' a').on('mouseenter',mouseenterFn);$('.elementor-element[data-id='+id_scope+'] svg, .'+elementSettings.id_svg_class+' a').on('mouseleave',mouseleaveFn);$('.elementor-element[data-id='+id_scope+'] svg, .'+elementSettings.id_svg_class+' a').on('touchstart',mouseenterFn);$('.elementor-element[data-id='+id_scope+'] svg, .'+elementSettings.id_svg_class+' a').on('touchend',mouseleaveFn)}else if(elementSettings.svg_trigger=='scroll'){$('#dce-svg-'+id_scope).attr('data-run','paused');active_scrollAnalysi('#dce-svg-'+id_scope)}
function getRandomValue(min,max){min=Math.ceil(min);max=Math.floor(max);return Math.floor(Math.random()*(max-min))+min}};$(window).on('elementor/frontend/init',function(){elementorFrontend.hooks.addAction('frontend/element_ready/dyncontel-svgdistortion.default',WidgetElements_SvgDistortionHandler)})})(jQuery)
