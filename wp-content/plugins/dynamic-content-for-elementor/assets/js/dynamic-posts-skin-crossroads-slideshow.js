...
var Widget_DCE_Dynamicposts_crossroadsslideshow_Handler = function ($scope, $) {
    ...
    class Slide {
        ...
        setCenter() {
            this.isCenter = true;
            this.DOM.el.classList.add('dce-grid__item--center');
            this.DOM.title.classList.add('dce-grid__item--center');
            TweenMax.set([this.DOM.el, this.DOM.title], {opacity: 1});
        }
        setRight() {
            this.isRight = this.isCenter = false;
            this.isLeft = true;
            this.DOM.el.classList.add('dce-grid__item--right');
            this.DOM.title.classList.add('dce-grid__item--right');
            TweenMax.set([this.DOM.el, this.DOM.title], {opacity: 1});
        }
        setLeft() {
            this.isLeft = this.isCenter = false;
            this.isRight = true;
            this.DOM.el.classList.add('dce-grid__item--left');
            this.DOM.title.classList.add('dce-grid__item--left');
            TweenMax.set([this.DOM.el, this.DOM.title], {opacity: 1});
        }
        reset() {
            ...
            this.DOM.title.classList = 'dce-grid__item dce-grid__item--title';
            this.DOM.el.classList = 'dce-grid__item dce-grid__item--slide';
        }
        animateElementsOut(contentItem) {
            return new Promise((resolve, reject) => {
                ...
                if ( this.isCenter ) {
                    ...
                    const contentItemImg = contentItem.querySelector('.dce-img--content');
    ...
    class Slideshow {
        constructor(el) {
            this.DOM = {el: el};
            // The titles
            this.DOM.titlesWrap = this.DOM.el.querySelector('.dce-titles-wrap');
            this.DOM.titlesInner = this.DOM.titlesWrap.querySelector('.dce-grid--titles');
            this.DOM.titles = [...this.DOM.titlesInner.querySelectorAll('.dce-grid__item--title')];

            // The slides instances
            this.slides = [];
            [...this.DOM.el.querySelectorAll('.dce-grid__item--slide')].forEach((slide, pos) => this.slides.push(new Slide(slide, this.DOM.titles[pos])));
            ...
            // Content Items
            this.DOM.contentItems = [...document.querySelectorAll('.dce-content__item')];

            // Areas (left, center, right) where to attach the navigation events.
            this.DOM.interaction = {
                left: document.querySelector('.dce-grid__item--left'),
                center: document.querySelector('.dce-grid__item--center'),
                right: document.querySelector('.dce-grid__item--right')
            };
        ...
        initEvents() {
            ...
            this.DOM.contentItems.forEach(item => {
                item.querySelector('.dce-img-wrap--content').addEventListener('click', () => this.closeSlide());
            });
        ...
        toggleSlide(action) {
            ...
            this.DOM.el.classList[action === 'open' ? 'add' : 'remove']('dce-content-open');
            ...
            if ( action === 'open' ) {
                contentItem.classList.add('dce-content__item--current');
            }
            Promise.all(promises).then(() => {
                if ( action === 'close' ) {
                    contentItem.classList.remove('dce-content__item--current');
                }
                this.isAnimating = false;
            });
        }
    ...
    // Revealer element
    const revealer = new Revealer(document.querySelector('.dce-revealer__inner'));
    // Initialize the slideshow
    new Slideshow(document.querySelector('.dce-grid--slideshow'));

    // Preload all the images in the page
    imagesLoaded(document.querySelectorAll('.dce-img'), {background: true}, () => document.body.classList.remove('loading'));
...
jQuery(window).on('elementor/frontend/init', function () {
	elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamicposts-v2.crossroadsslideshow', Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);
	elementorFrontend.hooks.addAction('frontend/element_ready/dce-woo-products-cart.crossroadsslideshow', Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);
	elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamic-woo-products.crossroadsslideshow', Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);
	elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamic-show-favorites.crossroadsslideshow', Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);
	elementorFrontend.hooks.addAction('frontend/element_ready/dce-my-posts.crossroadsslideshow', Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);
