var scene3d_istance=null;var Widget_DCE_Dynamicposts_3d_Handler=function($scope,$){var elementSettings=dceGetElementSettings($scope);var scene3d=$scope.find('.dce-posts-container.dce-skin-3d');var postsList=$scope.find('.dce-3d-wrapper .dce-item-3d');var is3dEnabled=!1;var isAnimating=!1;var elementsQuantity=postsList.length;var camera,scene,renderer;var controls;var objects=[];var targets={fila:[],circle:[],sphere:[],helix:[],grid:[]};var targets1={fila:[],circle:[],sphere:[],helix:[],grid:[]};var mouseX=0,mouseY=0;var windowHalfX=window.innerWidth/2;var windowHalfY=window.innerHeight/2;var targetSelect;var panelWidth=Number(elementSettings[dceDynamicPostsSkinPrefix+'size_plane_3d'])||320;var panelSpace=400;var distanza=1000;var diametro=(elementsQuantity*(panelWidth+panelSpace))/Math.PI;var raggio=diametro/2;var camDefaultY=300;var enableBlur=Boolean(elementSettings[dceDynamicPostsSkinPrefix+'blur_depth_3d'])||!1
var positionType=elementSettings[dceDynamicPostsSkinPrefix+'type_3d']||'circle';var positioning=targets.circle;if(positionType=='fila'){positioning=targets.fila}
...
for(var i=0;i<objects.length;i ++){targets.circle.push(rotationCalculation(i,raggio));targets1.circle.push(rotationCalculation(i,raggio+distanza));step+=(2*Math.PI)/elementsQuantity}
...
controls.enableDamping=!0;controls.dampingFactor=0.05;controls.enableZoom=!1;controls.autoRotate=!1;controls.screenSpacePanning=!0;controls.maxPolarAngle=Math.PI/2;controls.maxPolarAngle=Math.PI/1.7;controls.addEventListener('change',render);camReset();transform(positioning,2000);document.addEventListener('mousemove',onDocumentMouseMove,!1);window.addEventListener('resize',onWindowResize,!1);if(elementSettings[dceDynamicPostsSkinPrefix+'mousewheel_3d']){scene3d.on("mousewheel DOMMouseScroll",onMouseWheel)}
addElementsEvents();setTimeout(function(){Widget_DCE_Dynamicposts_base_Handler($scope,$)},300)}
let scrollStopAtEnd=elementSettings[dceDynamicPostsSkinPrefix+'mousewheel_3d_stop_at_end']==='yes';function onMouseWheel(event){var delta=event.originalEvent.wheelDelta/30||-event.originalEvent.detail;if(!isAnimating){if(delta<-1){if(positionType=='circle'){currentIndex+=1;if(currentIndex>=elementsQuantity){currentIndex=0;if(scrollStopAtEnd){scene3d.off("mousewheel DOMMouseScroll",onMouseWheel);return}}}else if(positionType=='fila'){currentIndex=currentIndex>0?currentIndex-1:0}
centerItem(currentIndex,1000)}else if(delta>1){if(positionType=='circle'){currentIndex=currentIndex>0?currentIndex-1:elementsQuantity-1}else if(positionType=='fila'){currentIndex+=1;if(currentIndex>=elementsQuantity){currentIndex=0;if(scrollStopAtEnd){scene3d.off("mousewheel DOMMouseScroll",onMouseWheel);return}}}
centerItem(currentIndex,1000)}}
event.preventDefault()}
function rotationCalculation(i,r){var theta=((Math.PI*2)/elementsQuantity);var angle=(theta*i)+(Math.PI/2);var object=new THREE.Object3D();object.position.x=r*Math.cos(angle);object.position.y=0;object.position.z=r*Math.sin(angle);return object}
...
controls.enableRotate=!0;is3dEnabled=!1;gsap.to(camera.position,{duration:1,ease:"power3.out",x:xTarget,y:yTarget,z:zTarget,});gsap.to(controls.target,{duration:1,ease:"power3.out",x:0,y:0,z:0,});for(var i=0;i<objects.length;i ++){objects[i].element.childNodes[1].style.display='block';if(enableBlur)blurObject(objects[i],!0)}}
function panCam(xTarget,yTarget,zTarget,xTarget1,yTarget1,zTarget1,tweenDuration){$scope.find('.dce-3d-navigation').addClass('dce-pancam-item');for(var i=0;i<objects.length;i ++){if(i==currentIndex){objects[i].element.childNodes[1].style.display='none';if(enableBlur)
gsap.to(objects[i].element,{duration:1,ease:"power3.inOut",filter:"blur(0px)",})}else{objects[i].element.childNodes[1].style.display='block';if(enableBlur)
...
function render(){var time=Date.now()*0.001;var rx=Math.sin(time*0.7)*0.5,ry=Math.sin(time*0.3)*0.5,rz=Math.sin(time*0.2)*0.5;for(var i=0;i<objects.length;i ++){var object=objects[i];var distance=camera.position.z-object.position.z;if(!is3dEnabled){if(enableBlur){blurObject(object)}}
...
function blurObject(object,animated=!1){var point1=camera.matrixWorld.getPosition().clone();var point2=object.position;var distance=(point1.distanceTo(point2)/distanza)-1;distance=distance*(3*distance);if(animated){gsap.to(object.element,{duration:0.6,webkitFilter:"blur("+distance.toFixed(2)+"px)",filter:"blur("+distance.toFixed(2)+"px)",})}else{gsap.set(object.element,{webkitFilter:"blur("+distance.toFixed(2)+"px)",filter:"blur("+distance.toFixed(2)+"px)",})}}};jQuery(window).on('elementor/frontend/init',function(){elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamicposts-v2.3d',Widget_DCE_Dynamicposts_3d_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamic-woo-products.3d',Widget_DCE_Dynamicposts_3d_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamic-show-favorites.3d',Widget_DCE_Dynamicposts_3d_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-my-posts.3d',Widget_DCE_Dynamicposts_3d_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-sticky-posts.3d',Widget_DCE_Dynamicposts_3d_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-search-results.3d',Widget_DCE_Dynamicposts_3d_Handler)})
