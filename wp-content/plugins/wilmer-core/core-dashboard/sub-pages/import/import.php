...
class WilmerCoreImport {
	...
	public function set_import_url() {
		$params = WilmerCoreDashboard::get_instance()->get_import_params();
	...
	public function import_action() {
				...
				if ( wp_verify_nonce( $_POST['options']['nonce'], 'mkdf_cd_import_nonce' ) ) {
					...
					switch ($_POST['options']['action']):
						...
						case 'menu-settings':
							...
							break;
						case 'rev-slider':
							if (wilmer_core_is_revolution_slider_installed()) {
								$this->rev_slider_import($demo);
							} else {
								$this->set_status('success');
								$this->set_data('type', 'options');
								$this->set_message(esc_html__('Revolution Slider isn\'t installed', 'wilmer-core'));
							}
							break;
						case 'content':
							...
							if($content_start) {
								if (!WilmerCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									wilmer_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'wilmer-core'));
									exit;
								}
							}
			...
			wilmer_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	function file_content( $path ) {
        ...
        if ( 200 !== intval( $response_code ) ) {
			$this->set_message($response["response"]['message'] . ' ' . esc_html__('Please contact support', 'wilmer-core'));
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $wilmer_widget_id => $wilmer_widget_data) {
				update_option('widget_' . $wilmer_widget_id, $wilmer_widget_data);
			}
			$ws = $this->import_sidebars_widgets($widgets);
			if($ws) {
				$this->set_message(esc_html__('Widgets are set for proper sidebar', 'wilmer-core'));
				$this->set_data('type', 'options');
				$this->set_status('success');
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$wilmer_sidebars = get_option( "sidebars_widgets" );
		unset( $wilmer_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$wilmer_sidebars = array_merge( (array) $wilmer_sidebars, (array) $data['sidebars'] );
			unset( $wilmer_sidebars['wp_inactive_widgets'] );
			$wilmer_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $wilmer_sidebars );
			$wilmer_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $wilmer_sidebars );
			return true;
		} else {
			return false;
		}
	...
	public function import_options( $file ) {
		...
		$current_options = get_option(WILMER_CORE_OPTIONS_NAME);
		if($options){
			if($current_options != $options) {
				$result = update_option(WILMER_CORE_OPTIONS_NAME, $options);
				if ($result) {
					$this->update_options_after_import($file);
					$this->set_status('success');
					$this->set_data('type', 'options');
					$this->set_message(esc_html__('Options imported successfully', 'wilmer-core'));

					$this->update_options_after_import($file);

				} else {
					$this->set_status('error');
					$this->set_message(esc_html__('Problem occurred during options import', 'wilmer-core'));
				}
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Options are already imported', 'wilmer-core'));
			}
		}
        if(wilmer_mikado_is_plugin_installed('elementor')){
	...
	public function import_menu_settings( $file ) {
		...
		if($menus_data) {
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message( esc_html__( 'Menus set for proper locations', 'wilmer-core' ) );
		} else {
			$this->set_status('error');
			$this->set_message( esc_html__( 'Problem during menus location set', 'wilmer-core' ) );
		}
	}
	...
	public function import_content( $file, $xml, $attachments, $post_id) {
		ob_start();
		require_once( WILMER_CORE_ABS_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
...
//		}
//
		if(wilmer_core_is_woocommerce_installed()) {
		...
		$wilmer_import = new WP_Import();
		set_time_limit( 0 );

		$wilmer_import->fetch_attachments = $attachments;
		$returned_value                  = $wilmer_import->import( $file . $xml );
		...
		if ( is_wp_error( $returned_value ) ) {
			...
			$this->set_message( esc_html__( 'An error occurred during content import', 'wilmer-core' ) );
		} else {
			$this->set_status('success');
			$this->set_data('type', 'content');
			$this->set_data('posts', $this->imported_posts);
			$this->set_message( esc_html__( 'File imported successfully', 'wilmer-core' ) . ' ' . $xml );
	...
	public function rev_sliders() {
		$rev_sldiers = array(
            'main-home.zip',
            'real-estate.zip',
            'business-home-slider-1.zip',
            'landing-slider-2.zip',
            'architecture.zip',
            'projects-home.zip',
            'landing-slider-3.zip',
            'construction.zip',
            'landing-slider-1.zip',
            'business-home-slider-2.zip',
			'about-me-slider.zip',
			'metro-showcase.zip'
		...
		if($files_created) {
			...
			foreach ($rev_sliders as $rev_slider) {
				...
				if (!$rev_results['success']) {
					$this->set_status('error');
					$this->set_message(esc_html__('Error while importing rev sliders', 'wilmer-core'));
					exit;
				}
			}
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Rev sliders imported successfully', 'wilmer-core'));
		} else {
			$this->set_status('error');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Files don\'t exist', 'wilmer-core'));
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( WILMER_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( WILMER_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return WILMER_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'mkdf_cd_import_nonce' ) ) {
				...
				$html = wilmer_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					wilmer_core_ajax_status( 'success', '', $html);
				} else {
					wilmer_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'wilmer-core' ), '');
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = WilmerCoreSystemInfoPage::get_instance()->get_system_info();
